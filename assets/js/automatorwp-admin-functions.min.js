...
var select2_args={ajax:{url:ajaxurl,dataType:'json',delay:250,type:'POST',data:function(params){return{q:params.term,page:params.page||1,action:$this.data('action'),nonce:automatorwp_admin_functions.nonce,table:$this.data('table'),option_none:$this.data('option-none'),option_none_value:$this.data('option-none-value'),option_none_label:$this.data('option-none-label'),option_custom:$this.data('option-custom'),option_custom_value:$this.data('option-custom-value'),option_custom_label:$this.data('option-custom-label'),}},processResults:automatorwp_select2_ajax_process_results},escapeMarkup:function(markup){return markup},templateResult:automatorwp_select2_ajax_template_result,theme:'default automatorwp-select2',placeholder:($this.data('placeholder')?$this.data('placeholder'):''),allowClear:!0,multiple:($this[0].hasAttribute('multiple'))};$this.automatorwp_select2(select2_args)}
...
str+=line+'\r\n'}
return str}
function automatorwp_insert_at_caret(input,content){input.each(function(){if(document.selection){this.trigger('focus');var sel=document.selection.createRange();sel.text=content;this.trigger('focus')}else if(this.selectionStart||this.selectionStart=='0'){var startPos=this.selectionStart;var endPos=this.selectionEnd;var scrollTop=this.scrollTop;this.value=this.value.substring(0,startPos)+content+this.value.substring(endPos,this.value.length);this.trigger('focus');this.selectionStart=startPos+content.length;this.selectionEnd=startPos+content.length;this.scrollTop=scrollTop}else{this.value+=content;this.trigger('focus')}})}
function automatorwp_set_caret_at_end(input){input=input[0];var length=input.value.length;if(document.selection){input.trigger('focus');var oSel=document.selection.createRange();oSel.moveStart('character',-length);oSel.moveStart('character',length);oSel.moveEnd('character',0);oSel.select()}else if(input.selectionStart||input.selectionStart=='0'){input.selectionStart=length;input.selectionEnd=length;input.trigger('focus')}}