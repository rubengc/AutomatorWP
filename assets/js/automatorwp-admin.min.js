...
item.slideUp('fast',function(){item.remove()})});$('body').on('click','.automatorwp-recommended-integrations-label a',function(e){e.preventDefault();$(this).closest('.automatorwp-recommended-integrations').find('.automatorwp-integrations').slideToggle('fast')});$('body').on('click','.automatorwp-add-filter',function(e){var button=$(this);var box=button.closest('.postbox');var items_list=box.find('.automatorwp-automation-items');var automation_id=$('#object_id').val();var item_type=(box.attr('id')==='automatorwp_triggers'?'trigger':'action');var type='filter';var last_position=items_list.find('.automatorwp-automation-item input.position').last();var position=0;if(last_position.length){position=parseInt(last_position.val())+1}else{position=parseInt(items_list.find('.automatorwp-automation-item').length-1)}
...
row.removeClass('is-option-change')});$('body').on('click','.automatorwp-automation-item-label > .automatorwp-option',function(e){var item=$(this).closest('.automatorwp-automation-item');var option=$(this).data('option');var option_form=item.find('.automatorwp-option-form-container[data-option="'+option+'"]');var custom_input_row=option_form.find('.automatorwp-selector-custom-input');if(custom_input_row!==undefined){var selector_row=custom_input_row.prev('.cmb-row');if(selector_row!==undefined){selector_row.addClass('is-option-change');selector_row.find('select.select2-hidden-accessible').trigger('change')}}});$('.automatorwp-selector-custom-input').each(function(){var selector_row=$(this).prev('.cmb-row');if(selector_row.find('select').val()==='custom'){$(this).show()}})})(jQuery);function automatorwp_update_items_position(element){var $=$||jQuery;var items=element.find('.automatorwp-automation-item');var items_order={};var items_order_for_tags=[];var current_position=0;items.each(function(index,value){$(this).find('input.position').val(index);if(!$(this).hasClass('automatorwp-automation-item-filter')){$(this).find('.automatorwp-automation-item-position').html((current_position+1)+'.');current_position++}
