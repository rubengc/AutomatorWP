 ...
 * Functions
 *
 * @package     AutomatorWP\Ninja_Forms\Functions
 * @since       1.0.0
 */
 ...
 * @return array
 */
function automatorwp_ninja_forms_options_cb_form( $field ) {
    ...
    if( ! empty( $value ) ) {
        ...
        foreach( $value as $form_id ) {
            ...
            $options[$form_id] = automatorwp_ninja_forms_get_form_title( $form_id );
        }
    }
 ...
 * @return string|null
 */
function automatorwp_ninja_forms_get_form_title( $form_id ) {
 ...
 * @return array
 */
function automatorwp_ninja_forms_get_form_fields_values( $data ) {
 ...
 * @return string
 */
function automatorwp_ninja_forms_parse_automation_tags( $parsed_content, $replacements, $automation_id, $user_id, $content ) {
    ...
    foreach( $triggers as $trigger ) {
        ...
        if( $trigger_args['integration'] !== 'ninja_forms' ) {
            continue;
        }
...
add_filter( 'automatorwp_parse_automation_tags', 'automatorwp_ninja_forms_parse_automation_tags', 10, 5 );